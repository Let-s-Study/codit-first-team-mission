...
export function HabitTracker({ habits, records}){
    let active = false;
    ...
    return (        
        <div className={style.container}>
            <h1 className={style.title}>습관 기록표</h1>
            <table className={style.wrap}>
                <thead>
                    <tr className={style.daysContainer}>
                    <th className={style.days} aria-label="습관명" />
                    {DAYS.map((d, dIdx) => (
                        <th key={d} className={style.day} scope="col" aria-colindex={dIdx + 2}>
                        {d}
                        </th>
                    ))}
                    </tr>
                </thead>
                <tbody className={style.tracker}>
                    {habits.map((h, rowIndex) => (
                    <tr key={h.id}>
                        <th className={style.habit} scope="row">
                        {h.text}
                        </th>
                        {DAYS.map((_, dIdx) => {
                        if (records && records[h.text]) {
                        active = Boolean(records[h.text][dIdx]);
                        }
                        return (
                            <td
                            key={dIdx}
                            ...
                            >
                            {active ? (
                                <Sticker fill={STICKER_COLOR[rowIndex % STICKER_COLOR.length]} />
                            ) : (
                                <Sticker />
                            )}
                            </td>
